...
function render() {
    ...
    if (!input) {
        ...
        const firstLineCord = canvas.height / 2 - (0.5 * lines.length - 0.5) * lineHeight;
        lines.forEach((line, index) => {
            ctx.fillText(line, canvas.width / 2, firstLineCord + index * lineHeight);
        });
    }
    downloadBtn();
}
